        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            {interaction.channel && (
              <span className="text-xs text-gray-600 dark:text-white/70">
                · {humanize(interaction.channel)}
              </span>
            )}

            {interaction.outcome && (
              <span className="text-xs text-gray-600 dark:text-white/70">
                · {humanize(interaction.outcome)}
              </span>
            )}

            {interaction.statusFilterFrom && interaction.statusFilterTo && (
              <span className="text-xs text-gray-600 dark:text-white/70">
                · {humanize(interaction.statusFilterFrom)} → {humanize(interaction.statusFilterTo)}
              </span>
            )}

            {interaction.pipelineStageFrom && interaction.pipelineStageTo && (
              <span className="text-xs text-gray-600 dark:text-white/70">
                · {humanize(interaction.pipelineStageFrom)} → {humanize(interaction.pipelineStageTo)}
              </span>
            )}
          </div>
          ...
          {details && Object.keys(details).length > 0 && (
            <div className="mt-3 grid grid-cols-1 gap-2 text-xs">
              {details.stageChange && (
                <DetailRow label="Stage Change" value={details.stageChange} />
              )}
              {details.statusChange && (
                <DetailRow label="Status Change" value={details.statusChange} />
              )}
...
function buildInteractionDetails(interaction: LeadInteraction) {
  ...
  if (interaction.pipelineStageFrom && interaction.pipelineStageTo) {
    const fromLabel = (STAGE_META as any)[interaction.pipelineStageFrom]?.label ?? humanize(interaction.pipelineStageFrom);
